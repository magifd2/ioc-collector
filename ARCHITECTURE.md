...
`--language`（`-l`）オプションで BCP 47 言語コードを指定できます（デフォルト: `ja`）。

**なぜ言語オプションが必要か:**

インシデント調査の対象は国際的なケースが多く、英語の調査ソースを扱う場合でも、日本語チームに向けたレポートと英語チームに向けたレポートの両方を生成したい場面があります。また、ツールを英語圏のユーザーが使う場合にも対応できます。

**実装の設計:**

| 対象 | 言語の影響 |
|---|---|
| `research()` のシステムプロンプト | `Write all text content in the following language (BCP 47 code): {language}` を追記 |
| `extract_report()` のシステムプロンプト | 同上。抽出結果のテキストフィールドを指定言語で生成させる |
| Markdown セクション見出し | `_HEADERS` 辞書から言語に対応した見出しを選択（`ja`/`en`、その他は英語フォールバック）|

Gemini への指示は BCP 47 コードをそのまま渡す設計にしています。モデルは BCP 47 を解釈できるため、`ja`（日本語）・`en`（英語）以外のコード（`zh`・`ko`・`fr` 等）でも Gemini レベルでは対応しています。セクション見出しは現在 `ja`/`en` のみ対応しており、その他は英語フォールバックとなります。

---

### 3.8 グラウンディングソース URL の解決

`research()` が返すグラウンディングメタデータ（`grounding_chunks[].web.uri`）には、Vertex AI のリダイレクト URL（`vertexaisearch.cloud.google.com/grounding-api-redirect/...`）が含まれることがあります。

**なぜ解決が必要か:**

リダイレクト URL をそのまま STIX や Markdown の参照として使うと、以下の問題が生じます。

| 問題 | 内容 |
|---|---|
| 可読性 | URL からどのページか判断できない |
| 永続性 | Vertex AI 側のリダイレクトサービスが終了すると無効になる可能性がある |
| 信頼性 | STIX の参照は本来、追跡可能な元の URL であるべき |
...
リダイレクト URL
    │
    ▼
urllib.request.urlopen（タイムアウト 5 秒）
    │
    ├── 成功 → response.url（最終到達 URL）を使用
    │
    └── 失敗（タイムアウト/ネットワークエラー等）
            │
            └── 元のリダイレクト URL をそのまま使用（フォールバック）
```

**フォールバックを設けた理由:**

リダイレクト URL を完全に除外すると参照が空になり、情報がゼロになります。機能するリダイレクトリンクは、「URL が存在しない」よりも常に価値があります。解決できない場合でも、アナリストがリンクをクリックすれば実際のページに到達できます。

**stdlib（`urllib`）を使う理由:**

`httpx` や `requests` など追加ライブラリを使わず stdlib の `urllib.request` で実装しています。依存を最小化し、インストール環境の差異を排除するためです。
...
class IncidentReport(BaseModel):
    ...
    references: list[ReferenceEntry]  # 参考情報（タイトルと URL の組）
```

`references` を `list[str]` ではなく `list[ReferenceEntry]` にしているのは、Pydantic のフィールドスキーマが `response_schema` 経由で Gemini に渡されるため、タイトルと URL を別フィールドで指定することで Gemini が正しく両方を抽出するよう誘導できるためです。Markdown 出力では `[タイトル](URL)` 形式のクリッカブルリンクとして展開されます。
...
すべての外部依存（Gemini API、ファイルシステム）はモックで差し替え、純粋なユニットテストとして実行できます。API キーや Google Cloud プロジェクトなしでテストが完結します。

| テストファイル | テスト対象 | テスト数 |
|---|---|---|
| `test_models.py` | Pydantic モデルのバリデーション | 9 |
| `test_defang.py` | デファング・リファング変換（パラメータ化テスト）| 31 |
| `test_gemini_client.py` | クライアント初期化・research()・言語オプション・URL解決 | 12 |
| `test_gemini_client_extract.py` | extract_report()・言語オプション | 6 |
| `test_gemini_client_errors.py` | エラーハンドリング・リトライ | 8 |
| `test_report.py` | Markdown 生成・言語切り替え・ファイル保存 | 15 |
| `test_stix_builder.py` | STIX Bundle 生成・パターン検証 | 10 |
| `test_main.py` | CLI エンドツーエンド（全依存モック）| 15 |
| `test_exceptions.py` | 例外クラス | 3 |
| **合計** | | **110** |
